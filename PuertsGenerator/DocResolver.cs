...
namespace Puerts
{
    public class DocResolver
    {
        ...
        private static Dictionary<AssemblyDefinition, DocResolver> _resolvers = new Dictionary<AssemblyDefinition, DocResolver>();
        ...
        public static DocResolver GetResolver(AssemblyDefinition assembly)
        {
            DocResolver resolver;
            if (!_resolvers.TryGetValue(assembly, out resolver))
            {
                resolver = _resolvers[assembly] = new DocResolver();
                if (!LoadXmlDocFrom(resolver, assembly.MainModule.FileName))
                {
        ...
        public static string GetTsDocument(MethodDefinition methodDefinition)
        {
            //return GetResolver(methodBase.DeclaringType.Assembly).GetDocBody(methodBase)?.ToJsDoc() ?? "";

            var docBody = GetResolver(methodDefinition.Module.Assembly).GetDocBody(methodDefinition);
        ...
        public static string GetTsDocument(FieldDefinition fieldDefinition)
        {
            //return GetResolver(fieldInfo.DeclaringType.Assembly).GetDocBody(fieldInfo)?.ToJsDoc() ?? "";

            var docBody = GetResolver(fieldDefinition.DeclaringType.Module.Assembly).GetDocBody(fieldDefinition);
        ...
        public static string GetTsDocument(PropertyDefinition propertyDefinition)
        {
            //return GetResolver(propertyInfo.DeclaringType.Assembly).GetDocBody(propertyInfo)?.ToJsDoc() ?? "";

            var docBody = GetResolver(propertyDefinition.DeclaringType.Module.Assembly).GetDocBody(propertyDefinition);
        ...
        public static string GetTsDocument(TypeDefinition typeDefinition)
        {
            //return GetResolver(type.Assembly).GetDocBody(type)?.ToJsDoc() ?? "";
            var docBody = GetResolver(typeDefinition.Module.Assembly).GetDocBody(typeDefinition);
        ...
        public DocBody GetDocBody(TypeDefinition typeDefinition)
        {
            if (typeDefinition.IsGenericInstance || typeDefinition.HasGenericParameters || !typeDefinition.IsPublic)
            {
                return null;
            }
            var xName = typeDefinition.FullName;
            DocBody body;
            _tdocs.TryGetValue(xName, out body);
            return body;
        }

        public DocBody GetDocBody(MethodDefinition methodDefinition)
        {
            if (methodDefinition.IsGenericInstance || !methodDefinition.IsPublic || methodDefinition.HasGenericParameters)
            {
                return null;
            }
            var declType = methodDefinition.DeclaringType;
            //_sb.Clear();
            _sb.Remove(0, _sb.Length);
            _sb.Append(declType.FullName);
            _sb.Append('.');
            _sb.Append(methodDefinition.Name);
            if (!ExtractMethodParamters(methodDefinition, _sb))
            {
                return null;
            }
        ...
        public DocBody GetDocBody(FieldDefinition fieldDefinition)
        {
            if (!fieldDefinition.IsPublic)
            {
                return null;
            }
            var declType = fieldDefinition.DeclaringType;
            var xName = declType.FullName + "." + fieldDefinition.Name;
            DocBody body;
            _fdocs.TryGetValue(xName, out body);
            return body;
        }

        public DocBody GetDocBody(PropertyDefinition propertyDefinition)
        {
            if (propertyDefinition.GetMethod == null || !propertyDefinition.GetMethod.IsPublic)
            {
                return null;
            }
            var declType = propertyDefinition.DeclaringType;
            var xName = declType.FullName + "." + propertyDefinition.Name;
            DocBody body;
            _pdocs.TryGetValue(xName, out body);
            return body;
        }

        private bool ExtractMethodParamters(MethodReference methodReference, StringBuilder sb)
        {
